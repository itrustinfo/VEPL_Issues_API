...
namespace PMTWebAPI.Controllers
{
    public class FinancialController : ApiController
    {
        ...
        //        //added on 16/04/2024 for android app
        //          string filenames = string.Empty;
        //          DataSet issue_all_attached = db.GetUploadedIssueDocuments(workpackaeissues.Tables[0].Rows[l]["Issue_Uid"].ToString());
        //        foreach (DataRow attached in issue_all_attached.Tables[0].Rows)
        //        {
        //            filenames += attached.ItemArray[1].ToString() + ",";
        ...
        [Authorize]
        [HttpPost]
        [Route("api/Financial/GetIssueDoc")]
        public IHttpActionResult GetIssueDoc()
        {
            List<Issue> finallist = new List<Issue>();
            var httpRequest = HttpContext.Current.Request;
            ...
            var transactionUid = Guid.NewGuid();
            var BaseURL = HttpContext.Current.Request.Url.ToString();
            string postData = "IssueUId=" + httpRequest.Params["IssueUId"] + ";DocName=" + httpRequest.Params["DocName"];

            db.WebAPITransctionInsert(transactionUid, BaseURL, postData, "");

            if (String.IsNullOrEmpty(httpRequest.Params["IssueUId"]) || String.IsNullOrEmpty(httpRequest.Params["DocName"]))
            {
                //db.WebAPITransctionUpdate(transactionUid, "Failure", "Error:Mandatory fields are missing");
                return Json(new
                {
            ...
            //added on 16/04/2024 for android app
            string fileName = ConfigurationManager.AppSettings["DocumentsPath"] + "\\Documents\\Issues\\" + httpRequest.Params["DocName"];
            string fileExtension = System.IO.Path.GetExtension(httpRequest.Params["DocName"]);
            string outPath = ConfigurationManager.AppSettings["DocumentsPath"] + "\\Documents\\Issues\\" + httpRequest.Params["DocName"].Remove(0, 2) + "_download" + fileExtension;
            ...
            {
                AttachedFileName = httpRequest.Params["DocName"],
                FileAsBase64 = converted_file
            };


            return Json(issue_attached_model);
        }
        ...
        /// <summary>
        /// this is new api that accepts user name as parameter
        /// </summary>
        /// <returns></returns>
        [Authorize]
        [HttpPost]
        ...
        public IHttpActionResult GetIssueList()
        {
            ...
            var transactionUid = Guid.NewGuid();
            var BaseURL = HttpContext.Current.Request.Url.ToString();
            string postData = "ProjectName=" + httpRequest.Params["ProjectName"] + ";WorkPackageName=" + httpRequest.Params["WorkPackageName"] + ";UserName=" + httpRequest.Params["UserName"];

            db.WebAPITransctionInsert(transactionUid, BaseURL, postData, "");

            if (String.IsNullOrEmpty(httpRequest.Params["ProjectName"]) || String.IsNullOrEmpty(httpRequest.Params["WorkPackageName"]) || String.IsNullOrEmpty(httpRequest.Params["UserName"]))
            {
                //db.WebAPITransctionUpdate(transactionUid, "Failure", "Error:Mandatory fields are missing");
                return Json(new
                {
            ...
            //
            DataSet dsUserDetails = db.getUserDetails_by_EmailID(httpRequest.Params["UserName"].ToString());
            bool isFilter = false;
            string UserUID = string.Empty;
            if (dsUserDetails.Tables[0].Rows.Count > 0)
            {
                if(dsUserDetails.Tables[0].Rows[0]["TypeOfUser"].ToString() != "PM")
                {
                    isFilter = true;
                    UserUID = dsUserDetails.Tables[0].Rows[0]["UserUID"].ToString();
                }
            ...
            DataTable workpackaeissues = db.getIssuesList_by_WorkPackageUID(new Guid(workpackage_uid)).Tables[0];

            DataTable dt = new DataTable();
            EnumerableRowCollection<DataRow> dataRows = null;
            if (isFilter)
            {
                dataRows = db.getIssuesList_by_WorkPackageUID(new Guid(workpackage_uid)).Tables[0].AsEnumerable().Where(r =>
                                                            r.Field<string>("Issued_User").ToLower() == UserUID.ToLower());
            ...
            for (int l = 0; l < workpackaeissues.Rows.Count; l++)
            {
                //if (workpackaeissues.Tables[0].Rows[l]["TaskUID"].ToString() == "00000000-0000-0000-0000-000000000000")
                //{
                ...
                if (!string.IsNullOrEmpty(workpackaeissues.Rows[l]["Actual_Closer_Date"].ToString()))
                {
                    assigned_date = Convert.ToDateTime(workpackaeissues.Rows[l]["Actual_Closer_Date"].ToString());
                }

                //added on 16/04/2024 for android app
                string filenames = string.Empty;
                DataSet issue_all_attached = db.GetUploadedIssueDocuments(workpackaeissues.Rows[l]["Issue_Uid"].ToString());
                foreach (DataRow attached in issue_all_attached.Tables[0].Rows)
                {
                    filenames += attached.ItemArray[1].ToString() + ",";
                ...
                Issue issue = new Issue()
                {
                    IssueUId = new Guid(workpackaeissues.Rows[l]["Issue_Uid"].ToString()),
                    IssueDescription = workpackaeissues.Rows[l]["Issue_Description"].ToString(),
                    Status = filenames,
                    IssueDate = Convert.ToDateTime(workpackaeissues.Rows[l]["Issue_Date"].ToString()),
                    ApprovedDate = assigned_date,
                    ReportingUser = db.getUserNameby_UID(new Guid(workpackaeissues.Rows[l]["Issued_User"].ToString())),
                    Remarks = workpackaeissues.Rows[l]["Issue_Remarks"].ToString()
                };
            ...
            return Json(finallist.ToList().OrderByDescending(a => a.IssueDate));
        }
